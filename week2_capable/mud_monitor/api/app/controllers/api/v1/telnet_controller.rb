module Api
  module V1
    class TelnetController < ApplicationController
      ...
      rescue_from TelnetLog::Store::NotFound, with: :render_not_found

      # GET /telnet?date=&session=&dir=&after=&limit=
      def index
        path    = store.path_for(date_param)
        records = path ? filter(TelnetLog::Parser.load(path).records) : []
        ...
        render json: {
          entries: page.map { |r| TelnetRecordSerializer.call(r) },
      ...
      # GET /telnet/stream?date=&session=&dir=&after=   (text/event-stream)
      def stream
        path = store.path_for!(date_param)
      ...
      def serve_stream(path)
        ...
        follower      = TelnetLog::Follower.new(path)
        ...
        loop do
          ...
          if new_records.any?
            new_records.each do |record|
              sse.write(TelnetRecordSerializer.call(record), event: "entry", id: record.seq)
              cursor = record.seq
            end
      ...
      def filter(records)
        ...
        records = records.select { |r| r.dir == params[:dir] } if params[:dir].present?
        records
      end
      ...
      def store
        @store ||= TelnetLog::Store.new(dir: cfg.telnet_dir, live_window: cfg.live_window)
      end
      ...
      def render_not_found(_error)
        render json: { error: { code: "not_found", message: "No telnet log for #{date_param}" } },
               status: :not_found
      end
    end
  end
